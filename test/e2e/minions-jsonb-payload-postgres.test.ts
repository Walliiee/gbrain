...
//
// What went wrong on a production brain (2026-08-28): a downstream patch
// wrapped the `$N::jsonb` binds in JSON.stringify. postgres.js already applies
// the jsonb serializer to a raw object, so the payload landed as a jsonb STRING
// SCALAR. Every SQL-level read then returned NULL — per-source scoping
...
const PROMPT = `You are a subagent ${LONE_HI}`;
const PROMPT_CLEAN = 'You are a subagent �';
...
d('minion JSONB payloads on real Postgres', () => {
  ...
  it('add() binds data as a jsonb object, so source scoping resolves', async () => {
    const job = await queue.add('facts-absorb', { sourceId: 'a-source', source_id: 'a-source' });
    ...
    // stored text visibly contains "sourceId":"a-source". That NULL is what
    // broke per-source backpressure, the synthesize daily-cap denominator, and
    // source-health job counts.
    expect(row.typ).toBe('object');
    expect(row.a).toBe('a-source');
    expect(row.b).toBe('a-source');
  });
