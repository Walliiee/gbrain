 ...
 * bit this brain on 2026-08-28, in opposite directions.
 *
 * 1. DOUBLE-ENCODE (the corruption). A local patch wrapped the `$N::jsonb`
 ...
 *    SCALAR — `"{\"sourceId\":\"shared\"}"` — instead of an object. Every
 ...
 *
 * PGLite caveat, same as test/links-timeline-jsonb-poison.test.ts: PGLite hides
 * the ORIGINAL double-encode crash, so lane 1 here locks the JS-side shape
 * rather than reproducing the Postgres-only failure. The upstream CI guard
 * scripts/check-jsonb-params.mjs is the other half of the pin — it fails on the
 * `$N::jsonb` + JSON.stringify shape statically.
 */
...
describe('sanitizeJsonbDeep', () => {
  test('well-forms lone surrogates in values, at any depth', () => {
    const out = sanitizeJsonbDeep({
      prompt: `You are Lens ${LONE_HI}`,
      nested: { deep: [{ text: `x${LONE_LO}y` }] },
    });
    expect(out.prompt).toBe('You are Lens �');
...
describe('minion payloads land as jsonb objects, not string scalars', () => {
  test('add() stores data as a jsonb OBJECT so SQL-level extraction works', async () => {
    const job = await queue.add('facts-absorb', { sourceId: 'shared', n: 1 });
    ...
    // whose stored text visibly contains "sourceId":"shared".
    expect(row.typ).toBe('object');
    expect(row.src).toBe('shared');
  });
  ...
  test('a lone surrogate in the payload does not abort the insert', async () => {
    ...
    expect(row.typ).toBe('object');
    expect(row.prompt).toBe('You are Lens �');
  });
