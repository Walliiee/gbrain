 ...
 * that night — and a capped source returns WITHOUT stamping it, handing the
 * baton to the next source in the loop. Destination by accident, not intent.
 *
...
import { runPhaseSynthesize, __testing } from '../src/core/cycle/synthesize.ts';
...
/** Phase opts are required-field-complete; only the allowlist inputs vary. */
function opts(extra: Record<string, unknown> = {}) {
  return { brainDir: '/tmp', dryRun: false, ...extra };
}
...
describe('runPhaseSynthesize — source_allowlist gate', () => {
  test('a source outside the allowlist is skipped as source_not_allowed', async () => {
    const res = await runPhaseSynthesize(
      ...
      opts({ sourceId: 'adaptig' }),
    );
    expect(res.status).toBe('skipped');
    expect((res.details as { reason: string }).reason).toBe('source_not_allowed');
    expect(res.summary).toContain('adaptig');
    // A shell script greps this line shape; keep the config key name in it.
    expect(res.summary).toContain('dream.synthesize.source_allowlist');
  });
  ...
  test('an explicit --input target bypasses the allowlist', async () => {
    const res = await runPhaseSynthesize(
      stubEngine(baseConfig({ 'dream.synthesize.source_allowlist': 'shared' })),
      opts({ sourceId: 'adaptig', inputFile: '/nonexistent/transcript.txt' }),
    );
    expect((res.details as { reason?: string }).reason).not.toBe('source_not_allowed');
  });
  ...
  test('an explicit --date target bypasses the allowlist', async () => {
    const res = await runPhaseSynthesize(
      stubEngine(baseConfig({ 'dream.synthesize.source_allowlist': 'shared' })),
      opts({ sourceId: 'adaptig', date: '2026-08-22' }),
    );
    expect((res.details as { reason?: string }).reason).not.toBe('source_not_allowed');
  });
});
