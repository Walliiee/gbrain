 ...
 *  page is no longer the current answer." `archived` is included as a LOCAL
 *  policy: this workspace tags done/retired pages `status: archived`, and this
 *  filter reads page-frontmatter `status` — a DIFFERENT column from the
 *  source-level archive flag (`sources.archived`) — so there is no collision.
 *  NOTE: this one value diverges from upstream PR #3061, which keeps the set to
 *  the four below to avoid author confusion. If the local file is ever replaced
 *  by the merged-upstream version, re-add `archived` here. */
