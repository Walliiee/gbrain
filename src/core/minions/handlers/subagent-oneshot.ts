...
/**
 * Per-call budget for the single provider call. 2026-09-05 (local patch):
 * env-overridable via GBRAIN_AI_ONESHOT_CALL_BUDGET_MS. The gateway's own
 * 300 s chat backstop (GBRAIN_AI_CHAT_TIMEOUT_MS) does not reach this
 * constant, and with the claude-cli path enforcing no output cap a ~30k-token
 * synthesis turn takes ~300 s at ~100 tok/s: jobs 686/687 (2026-09-03) hit
 * exactly this and survived only through the oneshot -> loop fallback. Still
...
export async function runSubagentOneshot(args: OneshotArgs): Promise<OneshotOutcome> {
  ...
  // CDX-9 task shapes: reflections/originals sub-trees of the allow-list, plus
  // the dream review queue (prompt Tasks E and F — candidate tasks and candidate
  // corrections). This is a SECOND fence on top of `matchesSlugAllowList`: the
  // allow-list says which paths exist at all, this says which of them a synthesis
  // child may write. `dream-cycle-summaries/*` is deliberately absent from both
  // lists — the orchestrator owns the index — and prompt Task C forbids touching
  // person pages.
