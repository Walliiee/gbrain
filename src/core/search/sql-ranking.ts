 ...
 * lifecycle hide value (`superseded`/`deprecated`/`retired`/`obsolete`/
 * `archived`). Like quarantine this is a search-only filter: a retired page
 * stays fetchable by slug (`get_page`) and in `list`, and its links resolve; it
 * is just not surfaced by keyword/vector search. Single source of truth:
 * `lifecycleFilterFragment` in `lifecycle.ts`.
 *
 * @returns raw SQL fragment, e.g.
 *   `AND p.deleted_at IS NULL AND NOT s.archived AND NOT (COALESCE(p.frontmatter, '{}'::jsonb) ? 'quarantine') AND NOT (lower(trim(COALESCE(p.frontmatter ->> 'status', ''))) IN ('superseded', 'deprecated', 'retired', 'obsolete', 'archived'))`
 */
