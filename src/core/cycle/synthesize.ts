...
export function generatedRecordStamp(
  ...
  return {
    defaults: {
      ...
      // #4337's immutable mirror. Seeded here for POSITION only, same as the two
      // above; `applyGeneratedStamp` resolves the real value from the row before
      // this function ever sees it, and `forced` below guarantees it.
    ...
    forced: {
      ...
      dream_generated: true,
      dream_cycle_date: cycleDate,
      // #4337: the immutable mirror moves in lockstep with `dream_cycle_date`
      // here because `cycleDate` is ALREADY the resolved stable date — either
      // the caller's explicit `--date`, or the row's own first cycle date that
      // `applyGeneratedStamp` read back. A rerender therefore re-emits the page's
      // original date instead of today's, and the DB path is protected
      // separately by the jsonb_build_object COALESCE in stampDreamProvenance.
      dream_created_cycle_date: cycleDate,
 ...
 *
 * When the caller does not name a cycle date, the row's OWN cycle date is the
 * authority — `stampDreamProvenance` writes the DB row before any reverse-render,
 * so a `--date 2026-08-22` backfill must not re-render as today. #4337 order:
 * `dream_created_cycle_date` (the immutable mirror) wins over the back-compat
 * `dream_cycle_date`, matching the COALESCE the DB stamp uses, so the two paths
 * can never disagree about which date a page was first generated on. Falling
 * through to `utcDate()` only when the row carries no stamp at all keeps a row
 * that lost its stamp to a put_page write-through renderable.
 */
export function applyGeneratedStamp(
  ...
  const fm = existing ?? {};
  const created = fm.dream_created_cycle_date;
  const legacy = fm.dream_cycle_date;
  const cycleDate = ctx.cycleDate
    ?? (typeof created === 'string' && created ? created : undefined)
 ...
 *
 * #4337 COEXISTS WITH THE STAMP, and the order matters. The contract's `forced`
 * block carries this run's `dream_cycle_date`, so applying it last would let a
 * re-synthesis rewrite a page's provenance to the maintenance run's date — the
 * exact regression #4337 fixed. The immutable cycle-date pair is therefore
 * appended AFTER `forced`, reading the pre-UPDATE row (SQL evaluates the SET
 * right-hand side against the old tuple): an existing `dream_created_cycle_date`
 * — else an existing `dream_cycle_date` — beats this run's date, and only a page
 * that carries neither takes `$3`.
 */
...
async function writeSummaryPage(
  ...
  const { defaults, forced } = generatedRecordStamp({
    ...
    // #4337 coexistence: the same cap that bounds the wikilink list bounds this.
    // An unbounded per-page ref list is the identical oversized-file/graph-hub
    // failure one layer up in the frontmatter, and the body's "Full output
    // provenance" section already states how to recover the complete set from
    // per-page `dream_generated` + `dream_cycle_date`. Over the cap, the cycle
    // itself is the honest single ref; at or under it, name every page.
