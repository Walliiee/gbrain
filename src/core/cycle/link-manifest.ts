 ...
 * corrections. Defined HERE rather than in synthesize.ts because three
 * modules need it — the prompt/stamp (synthesize.ts), the oneshot slug fence
 * (minions/handlers/subagent-oneshot.ts) and the self-consumption guard
 * below — and both of the others already import this module, so this is the
 * only shared node that introduces no import cycle. `_brain-filing-rules.json`
 * holds the matching allow-list glob; the two must stay in sync.
 */
...
function isDreamOutputSlug(slug: string, outputRoot: string): boolean {
  return slug.startsWith('dream-cycle-summaries/')
    // A proposal is an unreviewed claim. Offering one as a wikilink target
    // would let tonight's synthesis cite last night's unverified guess and
    // launder it into something that looks corroborated.
