...
export interface Check {
   ...
   *
   * A check that bails early — dependency absent, feature opt-in and off,
   * column not provisioned — has historically returned `status: 'ok'`,
   * because the status union has no third state. That is a false green: the
   * summary line says "All checks passed" while N checks never looked at
   * anything. Set this to `true` at any early-return site so the summary
   * stops vouching for it. `computeDoctorReport` also infers it from the
   * message for the sites that predate the field (see NOT_CHECKED_PATTERNS).
   *
...
export interface DoctorReport {
  ...
  /**
   * Names of checks that reported `ok` without running (see
   * `Check.not_checked`). Optional + additive; `schema_version` stays 2.
   * Present so a CI gate or monitor can assert coverage instead of trusting
   * a green tally that includes checks which never looked at anything.
   */
  not_checked?: string[];
}
...
export function computeDoctorReport(checks: Check[]): DoctorReport {
  ...
  // Post-pass: separate the checks that reported ok without running. Scores
  // are computed above and deliberately untouched — this only stops the
  // summary from counting a skip as evidence of health.
...
function outputResults(checks: Check[], json: boolean): boolean {
  ...
  // Checks that returned ok without running. Printed apart from the OK list
  // so a green tally can never be read as "everything was inspected".
  ...
  // The tally must never claim coverage it doesn't have. `ran` is the count
  // of checks that actually inspected something.
  const ran = report.checks.length - notChecked.size;
  // Only qualify the headline when there is something to qualify — a brain
  // with full coverage prints the exact same line it always has.
  const coverage =
