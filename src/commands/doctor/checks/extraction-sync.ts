...
export async function checkSyncFreshness(
  ...
  try {
    ...
    const sources = await engine.executeRaw<{
      ...
      //
      // LOCAL PATCH (not upstream at v0.46.19.0): the `federated` predicate
      // matches core/sources-ops.ts isFederated(), which is a strict `=== true`
      // — a source is federated only when it says so explicitly. Without this
